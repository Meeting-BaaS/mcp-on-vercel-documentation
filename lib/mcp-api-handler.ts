...
export function initializeMcpApiHandler(
  initializeServer: (server: McpServer, jwtToken: string) => void,
  ...
  return async function mcpApiHandler(
    ...
    await redisPromise;
    const url = new URL(req.url || "", "https://mcp-private.meetingbaas.com");
    ...
    // This is a public documentation server, no authentication required
    const emptyToken = "";
    ...
    if (url.pathname === "/mcp") {
      ...
      if (!statelessServer) {
        ...
        try {
          initializeServer(statelessServer, emptyToken);
        } catch (error) {
          console.error("Error initializing server:", error);
          // Continue without failing - this is a public server
        }
      }
      ...
      try {
        initializeServer(server, emptyToken);
      } catch (error) {
        console.error("Error initializing server:", error);
        // Continue without failing - this is a public server
      }
